...
from tsp.destroy_operators import adjacent_destroy, random_destroy
from tsp.repair_operators import greedy_insert
...
def solve_alns(
    ...
    if init is None:
        ordered = np.arange(1, data.num_cities).tolist()
        init = Solution.from_tour(data, ordered)
    ...
    return alns.iterate(init, select, accept, stop, data=data, **kwargs)
